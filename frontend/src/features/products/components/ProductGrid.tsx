...
export default function ProductGrid({ categoryId, categories }: ProductGridProps) {
    ...
    if (loading) return <section className="products"><div className="products-header">Товари</div><div className="products-muted">Завантаження товарів…</div></section>;
    if (error) return <section className="products"><div className="products-header">Товари</div><div className="products-error">Помилка: {error}</div></section>;
    if (filtered.length === 0) return <section className="products"><div className="products-header">Товари</div><div className="products-muted">Для цієї категорії товарів поки немає</div></section>;
                    ...
                    return (
                        <article key={p.id} className="product-card text-black">
                            <div className="product-card-image-wrap">
                                {imgSrc ? <img src={imgSrc} alt={p.name} className="product-card-image" loading="lazy" /> : <div className="product-card-image-fallback">Без фото</div>}
                            </div>
                            <div className="product-card-body">
                                <h3 className="product-card-title text-black" title={p.name}>{p.name}</h3>
                                <div className="product-card-bottom">
                                    <div className="product-card-price">{p.price.toLocaleString("uk-UA")} ₴</div>
                                    <div className="flex flex-col gap-2">
                                        <Link to={`/product/${p.id}`} className="product-card-btn text-center text-xs">Деталі</Link>
                                        <button
                                            onClick={() => handleAddToCart(p)}
                                            className="product-card-btn bg-[#F5A623] border-[#F5A623] hover:bg-white hover:text-[#F5A623] transition-colors text-xs"
                                        >
                                            Купити
                                        </button>
                                    </div>
