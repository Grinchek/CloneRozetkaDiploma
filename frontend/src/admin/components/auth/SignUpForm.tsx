...
import {
    useRegisterMutation,
    useLoginByGoogleMutation, // якщо Google на бекенді робить "логін/реєстрацію" одним ендпоінтом
...
export default function SignUpForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!isChecked) {
            setError("Потрібно погодитися з Terms and Conditions.");
            return;
        }

        try {
            // ✅ Формуємо FormData замість JSON
            const fd = new FormData();
            fd.append("userName", form.email);      // назви мають збігатися з DTO на бекенді
            ...
            if (avatar) {
                fd.append("avatar", avatar); // "avatar" має збігатися з RegisterFormDto.Avatar
            }
            ...
            setError(err?.data?.message ?? "Помилка реєстрації. Перевір дані.");
        }
    };
    ...
    const signUpWithGoogle = useGoogleLogin({
        onSuccess: async (tokenResponse: TokenResponse) => {
            setError(null);
            try {
                // Більшість бекендів роблять один ендпоінт: "LoginByGoogle" який і реєструє, якщо користувача нема
                ...
                if (result?.token) {
                    ...
                    setError("Google: сервер не повернув токен.");
                }
            } catch (err: any) {
                console.error("Google auth error:", err);
                setError(err?.data?.message ?? "Помилка Google-реєстрації/входу.");
            }
        },
        onError: (err) => {
            console.error("Google auth error:", err);
                  ...
                  Or
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="space-y-5">
                                {error && (
                                    <div className="p-3 text-sm rounded-lg bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-300">
                                        {error}
                                    </div>
                                )}
                          ...
                          <EyeCloseIcon className="fill-gray-500 dark:fill-gray-400 size-5" />
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="w-16 h-16 rounded-full bg-gray-100 dark:bg-white/10 overflow-hidden flex items-center justify-center">
                                            {avatarPreview ? (
                                                <img
                                                    src={avatarPreview}
                                                    alt="avatar preview"
