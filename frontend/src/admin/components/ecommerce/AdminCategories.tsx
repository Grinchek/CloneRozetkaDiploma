...
import { useDeleteCategoryMutation, useGetCategoriesPagedQuery } from "../../../features/categories/api/categoryApi.ts";
...
export default function CategoriesTable() {
    ...
    const { data, isLoading, isFetching, error, refetch } = useGetCategoriesPagedQuery({ page, pageSize });
    ...
    // якщо видалили останній елемент на сторінці — можна автоматично піти на попередню сторінку
    useEffect(() => {
        if (!data) return;
        if (page > 1 && data.items.length === 0) setPage((p) => p - 1);
    }, [data, page]);

    const askDelete = (id: number | string, name: string) => {
        setSelected({ id, name });
        setConfirmOpen(true);
    };
                ...
                <div className="max-w-full overflow-x-auto">
                    <Table>
                        ...
                        <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
                            {items.map((cat) => {
                                const imgSrc = cat.image ?  `${API_URL}/Images/${'100_'}${cat.image}` : "/images/placeholder.png";
                                    ...
                                    <TableRow key={cat.id}>
                                        <TableCell className="py-3">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="text-theme-sm font-medium text-gray-800 dark:text-white/90">{cat.name}</p>
                            ...
                            image: {cat.image || "-"}
                                        ...
                                        <TableCell className="py-3 text-theme-sm text-gray-500 dark:text-gray-400">{cat.id}</TableCell>
